<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <meta http-equiv="X-UA-Compatible" content="ie=edge" />
    <title>Corporate Law</title>
    <link
      href="https://fonts.googleapis.com/css?family=Roboto:400,700"
      rel="stylesheet"
    />
    <link
      href="https://fonts.googleapis.com/css?family=Poppins:400,700"
      rel="stylesheet"
    />

    <link rel="stylesheet" href="../css/bootstrap.css" />
    <link rel="stylesheet" href="../css/style.css" />
    <link rel="stylesheet" href="../css/all.min.css" />
    <link rel="stylesheet" href="../css/animate.min.css" />
    <link rel="stylesheet" href="../css/aos.css" />
  </head>
  <body>
    <!-- Navbar -->

    <section>
      <div class="container-flex fixed-top">
        <div class="row">
          <div class="col-12">
            <ul class="social nav bg-dark text-white justify-content-end">
              <li class="nav-item">
                <a class="nav-link" href="#"
                  >Connect with us on linkendin <i class="fab fa-linkedin"></i
                ></a>
              </li>
              <li class="nav-item">
                  <a class="nav-link">|</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link" href="#">Subscribe To AKD</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link">|</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link" href="#">AKDFocus Login </a>
                </li>
                <li class="nav-item">
                  <a class="nav-link">|</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link" href="#"><i class="fas fa-search"></i></a>
                </li>
            </ul>
          </div>
          <div class="col-12 border border-primary">
            <nav class="navm navbar navbar-expand-xl">
              <a class="nav-link" href="index"
                ><img src="../img/pravo_i_vaga.jpg"
              /></a>
              <button
                id="bt"
                class="navbar-toggler"
                type="button"
                data-toggle="collapse"
                data-target="#navbarSupportedContent"
                aria-controls="navbarSupportedContent"
                aria-expanded="false"
                aria-label="Toggle navigation"
              >
              <i class="fas fa-bars fa-2x" style="color:rgb(0, 0, 0);"></i>
              </button>
              <div
                class="collapse navbar-collapse justify-content-end"
                id="navbarSupportedContent"
              >
              <form class="form-inline">
                  <div class="bord">
                    <ul id="nav" class="nav navbar-nav">
                    <li class="nav-item">
                    <a class="nav-link" href="../index">Home</a>
                      </li>
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" href="../about_us" id="navbarDropdownMenuLink" 
                                aria-haspopup="true" aria-expanded="false">
                                About Us
                            </a>
                            <div class="drp dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                                <a class="itm dropdown-item" href="./our_history">Our History</a>
                                <a class="itm dropdown-item" href="./our_promise">Our Promise</a>
                            </div>
                        </li>
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" href="../service" id="navbarDropdownMenuLink" 
                                aria-haspopup="true" aria-expanded="false">
                                Service
                            </a>
                            <div class="drp dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                                <a class="itm dropdown-item" href="./banking">Banking</a>
                                <a class="itm dropdown-item" href="./it">IT</a>
                                <a class="itm dropdown-item" href="./corporate">Corporate</a>
                                <a class="itm dropdown-item" href="#">Energy</a>
                                <a class="itm dropdown-item" href="#">Litigation</a>
                                <a class="itm dropdown-item" href="#">Arbitration</a>
                                <a class="itm dropdown-item" href="#">Mediation/Conciliation</a>
                                <a class="itm dropdown-item" href="#">Science</a>
                            </div>
                        </li>
                        <li class="nav-item">
                        <a class="nav-link" href="../people">People</a>
                      </li>
                      <li class="nav-item">
                        <a class="nav-link" href="../postphp/news">News</a>
                      </li>
                      <li class="nav-item">
                        <a class="nav-link" href="../careers">Careers</a>
                      </li>
                      <li class="nav-item">
                        <a class="nav-link" href="../contact">Contact</a>
                      </li>
                    </ul>
                  </div>
                </form>
              </div>
            </nav>
          </div>
        </div>
      </div>
    </section>

<!-- About -->
<section>
  <div class="about conatiner-fluid">
    <div class="m row">
      <div class="col-lg-3">
        <img src="../img/arc.jpg" width="100%" height="100%">
      </div>
      <div class="col-lg-6" style="background-color:lightblue; height:2000px;">
        <h2 class="py-3">AKD Corporate</h2>
        <hr style="background-color:black; border-width: 5px;">
       <h6 class="my-4 text-justify ">AKD has a successful practice of handling disputes for a diverse range of respected and recognized national clients, including financial institutions such as banks and insurers and major corporations. AKD strongly believe that client respects professionally opinion. So we are considering our job as giving advices and law interpretations; believes and options we are leaving to others.</h6>
       
       <div class="p-3 groups">
       <div class="card mb-3" style="max-width: 950px;">
        <div class="row no-gutters">
          <div class="col-md-4">
            <img src="../img/arc.jpg" height="100%" class="p-3 card-img" alt="...">
          </div>
          <div class="col-md-8">
            <div class="card-body">
              <h5 class="card-title">COMMERCIAL LITIGATION</h5>
              <ul class="m-4 list-group">
                <li class="p-2 list-item">Debt disputes </li>
                <li class="p-2 list-item">Recovery of unpaid debt and enforcement of judgments </li>
                <li class="p-2 list-item">Contract Disputes </li>
                <li class="p-2 list-item">Disputes over interests in land and personal property </li>
                <li class="p-2 list-item">Banking commercial disputes </li>
              </ul> 
            </div>
          </div>
        </div>
      </div>

      <div class="card mb-3" style="max-width: 950px;">
        <div class="row no-gutters">
          <div class="col-md-4">
            <img src="../img/arc.jpg" height="100%" class="p-3 card-img" alt="...">
          </div>
          <div class="col-md-8">
            <div class="card-body">
              <h5 class="card-title">COMMERCIAL TRANSACTIONS</h5>
                <ul class="m-4 list-group">
                <li class="p-2 list-item">Maintenance and service agreements</li>
                <li class="p-2 list-item">Memoranda of understanding</li>
                <li class="p-2 list-item">Confidentiality agreements</li>
                <li class="p-2 list-item">Consultancy agreements</li>
                <li class="p-2 list-item">Real estate transfers and leases</li>
                <li class="p-2 list-item">License agreements</li>
                <li class="p-2 list-item">Distribution agreements</li>
                <li class="p-2 list-item">Shareholder, partnership, and joint venture agreements</li>
                <li class="p-2 list-item">Loan agreements, guarantees and securities</li>
              </ul> 
            </div>
          </div>
        </div>
      </div>

      <div class="card mb-3" style="max-width: 950px;">
        <div class="row no-gutters">
          <div class="col-md-4">
            <img src="../img/arc.jpg" height="100%" class="p-3 card-img" alt="...">
          </div>
          <div class="col-md-8">
            <div class="card-body">
              <h5 class="card-title">SPECIALIZED MARKET OPERATIONS</h5>
              <ul class="m-4 list-group">
                <li class="p-2 list-item">IT</li>
                <li class="p-2 list-item">Energy</li>
                <li class="p-2 list-item">Public procurement</li>
              </ul> 
            </div>
          </div>
        </div>
      </div>

    </div>
      </div>
      <div class="col-lg-3" style="background-color: lightblue;">
      </div>
    </div>
  </div>
</section>


<!-- Footer   -->

<footer class="contact_form">
    <div class="container-fluid">
    <div class="row p-3 mb-2 bg-light text-dark">
      <div class="col-md-12 justify-content-end">
          <ul class="nav justify-content-end">
              <li class="nav-item">
                <a class="nav-link active" href="#">Terms of Use</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="#">Privacy policy</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="#"><i class="far fa-copyright"></i> Copyright AKDLegal 2019</a>
              </li>
            </ul>
      </div>
    </div>
  </div>
</footer>

    <script src="../js/jquery-3.3.1.min.js"></script>
    <script src="../js/popper.min.js"></script>
    <script src="../js/bootstrap.bundle.js"></script>
    <script src="../js/index.js"></script>
    <script src="../js/aos.js"></script>
    <script>
        AOS.init();
        </script>
  </body>
</html>
